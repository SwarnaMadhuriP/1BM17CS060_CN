...
main (int argc, char *argv[])
{
  ...
  NodeContainer nodes,nodes1;
  nodes.Create (2);
  nodes1.Create(2);
  PointToPointHelper pointToPoint,p2p;
  pointToPoint.SetDeviceAttribute ("DataRate", StringValue ("8Mbps"));
  pointToPoint.SetChannelAttribute ("Delay", StringValue ("3ms"));
  p2p.SetDeviceAttribute ("DataRate", StringValue ("7Mbps"));
  p2p.SetChannelAttribute ("Delay", StringValue ("1ms"));
  NetDeviceContainer devices,device1;
  devices = pointToPoint.Install (nodes);
  device1=p2p.Install(nodes1);
  ...
  echoClient.SetAttribute ("PacketSize", UintegerValue (1024));
   ...
   UdpEchoClientHelper echoClient1(interfaces1.GetAddress (0), 11);
  echoClient.SetAttribute ("MaxPackets", UintegerValue (1));
  echoClient.SetAttribute ("Interval", TimeValue (Seconds (1.0)));
  echoClient.SetAttribute ("PacketSize", UintegerValue (1024));
